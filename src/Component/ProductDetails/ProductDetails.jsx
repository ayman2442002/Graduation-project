...
{product?.images.map((src)=> <img src={src} key={product.id} className='w-full'/> )}
  ...
  <button className='btn'>Add To Cart</button>
   ...
   <div className="row">
{relatedProducts.length>0 ? relatedProducts.map((product)=> (<div key={product.id} className='w-full md:w-1/3 lg:w-1/4 xl:w-1/6'>
...
</div>)):<div class="lds-spinner "><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>}
